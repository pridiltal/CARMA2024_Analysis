...
data_world <- read.csv( here::here("data", "1Digital_lms_world.csv"))
d<- data_world |> 
  mutate_at(vars(-date), as.character)

data_world <- d |> pivot_longer(
  cols = `Google.Classroom`:Skooler,
  names_to = c( "keyword"),
  values_to = "hits"
)
...
dataW <- data_world |>
  select(date, keyword, hits) |>
  mutate(keyword = factor(keyword, levels = keyword_ord))
dataW$date = as.Date(dataW$date, "%m/%d/%Y")
...
p1 <- dataW |>
  ...
  labs(subtitle = "Digital learning management systems", x = "")
...
data_world <- read.csv( here::here("data", "2mobile_phones_apps_world.csv"))
d<- data_world |> 
  mutate_at(vars(-date), as.character)
...
p2<- dataW |> as_tibble() |>
  ...
  theme(legend.position = "bottom", 
        ...
        title = element_text(size = 20) )+
  scale_colour_discrete(guide = "colourbar") + 
  labs(subtitle ="Systems built for use on basic mobile phones",
       x="")
...
write.csv(data_world,
          here::here("data", 
                     "3offline_functionality_world.csv"))



## ---- offline_functionality_analysis
keywords <-  c("Kolibri", "Rumie", "Ustad Mobile")
data_world <- read.csv( here::here( "data", "3offline_functionality_world.csv"))
d<- data_world |> 
  mutate_at(vars(-date), as.character)

data_world <- d |> pivot_longer(
  cols = Kolibri:Ustad.Mobile,
  names_to = c( "keyword"),
  values_to = "hits"
)
...
p3<- dataW |> as_tibble() |>
  ...
  scale_colour_discrete(guide = "colourbar") + 
  labs(subtitle ="Systems with strong offline functionality",
       x="")
...
#data_world <- purrr::map_df(keywords, get_info_world)
write.csv(data_world, here::here( "data", "4mooc_worldmooc_world.csv"))
...
keywords= c("Alison", "Canvas", "Coursera", "European Schoolnet Academy", 
            "EdX", "iCourse", "Future Learn", "Icourses", "TED-Ed Earth School",
            "Udemy", "XuetangX")
data_world <- read.csv( here::here( "data", "4mooc_world.csv"))

d<- data_world |> 
  mutate_at(vars(-date), as.character)

data_world <- d |> pivot_longer(
  cols = Canvas:XuetangX,
  names_to = c( "keyword"),
  values_to = "hits"
)
...
p4 <- dataW |> as_tibble() |>
  ...
  #scale_colour_viridis_d(guide = "colourbar", direction = -1) + 
  scale_colour_discrete(guide = "colourbar") + 
  labs(subtitle ="Massive Open Online Course (MOOC) platforms",
       x="")
...
keywords6 <- c("Quizlet","OneCourse", "Polyup", "SDG Academy Library")
...
#data_world <- purrr::map_df(keywords, get_info_world)
write.csv(data_world, here::here( "data", "5Self_directed_learning_world.csv"))
...
keywords= c("ABRA", "British Council", "Byju’s", "Code It", 
            ...
            "Polyup", "Quizlet", "SDG Academy Library", "Siyavula",
            "Smart History", "youtube")
data_world <- read.csv( here::here( "data", "5Self_directed_learning_world.csv"))

d<- data_world |> 
  mutate_at(vars(-date), as.character)

data_world <- d |> pivot_longer(
  cols = Quizlet:Smart.History,
  names_to = c( "keyword"),
  values_to = "hits"
)
...
p5 <-  dataW |> as_tibble() |>
  ...
  #scale_colour_viridis_d(guide = "colourbar", direction = -1) + 
  scale_colour_discrete(guide = "colourbar") + 
  labs(subtitle ="Self-directed learning content",
       x = "Time")
...
data_world <- bind_rows(trends1, trends21)
write.csv(data_world, here::here( "data", "6Mobile_reading_world.csv"))
...
data_world <- read.csv( here::here( "data", "6Mobile_reading_world.csv"))

d<- data_world |> 
  mutate_at(vars(-date), as.character)

data_world <- d |> pivot_longer(
  cols = Reads:Worldreader,
  names_to = c( "keyword"),
  values_to = "hits"
)
...
p6 <-  dataW |> as_tibble() |>
  ...
  #scale_colour_viridis_d(guide = "colourbar", direction = -1) + 
  scale_colour_discrete(guide = "colourbar") + 
  labs(subtitle ="Mobile reading applications",
       x="")
...
data_world <- bind_rows(trends1, trends21)
write.csv(data_world, here::here( "data", "7Collaboration_platforms_world.csv"))
...
keywords= c("Zoom", "Dingtalk", "Lark", "Hangouts Meet",
            "Teams", "Skype", "WhatsApp")
data_world <- read.csv( here::here( "data", "7Collaboration_platforms_world.csv"))
...
p7 <-  dataW |> as_tibble() |>
  ...
  #scale_colour_viridis_d(guide = "colourbar", direction = -1) + 
  scale_colour_discrete(guide = "colourbar") + 
  labs(subtitle ="Collaboration platforms that support live-video communication", x="")
...
data_world <- bind_rows(trends1, trends21)
write.csv(data_world, here::here( "data", "8Tools_for_teachers_world.csv"))

## ---- Tools_for_teachers_analysis

data_world <- read.csv( here::here( "data", "8Tools_for_teachers_world.csv"))
d<- data_world |> 
  mutate_at(vars(-date), as.character)
...
p8 <-  dataW |> as_tibble() |>
  ...
  #scale_colour_viridis_d(guide = "colourbar", direction = -1) + 
  scale_colour_discrete(guide = "colourbar") + 
  labs(subtitle ="Tools for teachers to create of digital learning content", x=""
  )
...
data_world <- bind_rows(trends1, trends21, trends31)
write.csv(data_world, here::here("data", "9External_repositories_DL_world.csv"))
...
data_world <- read.csv( here::here( "data", "9External_repositories_DL_world.csv"))

d<- data_world |> 
  mutate_at(vars(-date), as.character)

data_world <- d |> pivot_longer(
  cols = Brookings:UNHCR,
  names_to = c( "keyword"),
  values_to = "hits"
)
...
p9 <-  dataW |> as_tibble() |>
  ...
  # scale_colour_viridis_d(guide = "colourbar", direction = -1) + 
  scale_colour_discrete(guide = "colourbar") + 
  labs(subtitle ="External repositories of distance learning solutions",
       x = "")
...
write.csv(data_world, here::here("data", "10Online_proctoring_world.csv"))
...
data_world <- read.csv( here::here( "data", "10Online_proctoring_world.csv"))

d<- data_world |> 
  mutate_at(vars(-date), as.character)

data_world <- d |> pivot_longer(
  cols = Pearson.VUE:Talview,
  names_to = c( "keyword"),
  values_to = "hits"
)
...
p10 <-  dataW |> as_tibble() |>
  ...
  #scale_colour_viridis_d(guide = "colourbar", direction = -1) + 
  scale_colour_discrete(guide = "colourbar") + 
  labs(subtitle ="Tools for online proctoring",
       x = "Time")
